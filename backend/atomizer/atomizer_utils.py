...
def validate_llm_output(
    ...
    for raw_group in raw_groups:
        ...
        valid_fact_claims: list[AtomizedFactClaim] = []
        ...
        for raw_fact_claim in raw_fact_claims[:MAX_FACT_CLAIMS_PER_GROUP]:
            ...
            if has_unresolved_reference(claim_text):
                continue
            if not extract_numbers(claim_text).issubset(extract_numbers(sentence_context)):
                continue
            claim_key = normalize_text(claim_text)
            if claim_key in seen_claims:
                continue

            fact_claim = make_fact_claim(
                len(valid_fact_claims) + 1,
                claim_text,
    ...
    for sentence_text in sentence_items:
        if normalize_text(sentence_text) not in target_sentence_keys:
            continue
