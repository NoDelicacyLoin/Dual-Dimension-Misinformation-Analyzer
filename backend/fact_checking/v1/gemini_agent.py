...
def prepare_claim_for_fact_checking(raw_claim: str, use_query_rewrite: bool = True) -> ClaimPreparationResult:
    """
    Lightly guard against empty input and optionally rewrite the claim into a
    ...
    rewrite_instruction = (
        ...
        "Do not rewrite the claim. Return it unchanged."
    )

    prompt = f"""
You are preparing a user claim for a fact-checking retrieval system.

Return only the final claim text to use for fact-checking.
...
- For simple identity, occupation, authorship, membership, or category claims, evidence that names the entity and states a matching role or category is direct signal, not background.
- For spatial relation claims, do not treat "inside", "part of", "within", or "contains" as support for "adjacent to", "next to", or "bordering"; these are different relations.
