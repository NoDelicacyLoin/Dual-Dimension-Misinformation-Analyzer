...
def map_truth_score_to_verdict(factual_claim: EachFactualClaim) -> None:
    if factual_claim.truth_score is None:
        factual_claim.verdict = None
    elif factual_claim.truth_score >= 0.85:
        factual_claim.verdict = "True"
    elif factual_claim.truth_score >= 0.65:
        factual_claim.verdict = "Mostly True"
    elif factual_claim.truth_score >= 0.45:
        factual_claim.verdict = "Neutral"
    elif factual_claim.truth_score >= 0.25:
        factual_claim.verdict = "Mostly False"
    else:
        factual_claim.verdict = "False"
...
def aggregate_truth_score(factual_claim: EachFactualClaim) -> None:
    ...
    truth_score = NEUTRAL_SCORE + (MAX_SCORE_SHIFT * balance_score)
    if truth_score < 0.0:
        factual_claim.truth_score = 0.0
    elif truth_score > 1.0:
        factual_claim.truth_score = 1.0
    else:
        factual_claim.truth_score = truth_score
