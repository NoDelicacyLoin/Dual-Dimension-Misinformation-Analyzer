...
This led to an experimental post-Gemini background fallback. The idea was: if Gemini judges all selected evidence as background, try one more retrieval query before returning `insufficient_evidence`. The experiment was useful because it showed where the retrieval/filter boundary was weak.

The motivating example is:

`The Quiet only stars Hillary Clinton.`

The pre-Gemini filter selected Hillary Clinton sources and rated them as strong or usable, so the normal rewrite fallback did not trigger. Gemini correctly judged those sources as background because they did not discuss the film cast. This pattern is useful for analysis, but the later code cleanup narrowed recovery again so it would not become a chain of repeated rewrites.
...
| `fact_check_service` | Run the main claim search, call optional empty-search fallback, and call evidence judgment. |
| `recovery` | Add a rewrite fallback only for empty search, reset claim judgment when retrying, and stabilize borderline results. |
...
Post-Gemini all-background cases were kept conservative. The current code does not repeatedly rewrite just because Gemini judged selected evidence as background. It only tries a remaining query if one is already available from the recovery path; otherwise the claim remains `insufficient_evidence`. This preserves the rule that evidence sufficiency is decided after evidence is actually judged.
...
At this stage, the fact-checking service tried a small set of search queries for each claim:
...
This was useful for diagnosis, but later became redundant after the atomizer started producing standalone checkable claims with structured `entities`, `relation`, and `constraints`. The current main path is simpler: search the checkable claim directly. Rewrite is now only an optional empty-search fallback, and the structured claim fields live on `EachFactualClaim` rather than retrieval metadata.
...
### Long-Form News Input Limitation

Testing with a longer ISOT-style fake news example exposed an important limitation of the current product shape. The backend works best when the input is a short claim or a short paragraph containing a small number of factual claims. When a full news-style article is submitted, the atomizer can produce many claim groups and factual sub-claims. The result is technically complete, but it becomes too large and hard to read as a user-facing report.

This is not mainly a single fact-checking bug. It is a product-level limitation caused by the interaction between atomization, retrieval, aggregation, and presentation:

- the atomizer may split long text into too many claims without clearly ranking which claims are central to the article;
- repeated background or secondary claims can make the result page long and visually dense;
- the aggregate verdict can be diluted when a fake or misleading article contains several ordinary true background statements;
- each individual fact-checking card may be reasonable, but the full report can still fail to communicate the main misinformation issue clearly;
- the current result page design is suitable for claim-level review, but not yet for article-level summarization.

This suggests that long-form news analysis should eventually have a separate article-level layer. That layer should identify the main factual claims, distinguish central claims from background details, and summarize the most important evidence conflicts first. The current fact-checking branch can still be used as the claim-level engine, but a longer article needs claim prioritization and report compression before it becomes a polished product experience.

The same test also exposed several atomizer-level issues that should be handled before treating long-form article input as a finished product path.

First, the atomizer currently avoids pronoun resolution. This was originally a conservative choice because the atomizer should not invent facts. However, the retrieval branch now depends heavily on entity-relation search. If a sentence says `he`, `she`, `they`, `this`, or `that`, the fact-checking query may lose the actual subject from the previous sentence. For long articles, the atomizer should preserve enough local context to turn a factual claim into a searchable standalone claim. This does not mean freely rewriting the article; it means carrying over the clear subject from the nearby context when the sentence itself uses a pronoun.

Second, the atomizer needs claim prioritization. Long news articles contain many entities, dates, titles, actions, and background statements. Checking all of them is slow and can bury the claim the user actually cares about. A better long-form path should extract and rank core factual assertions before sending them to the retrieval branch. Secondary background claims can still be kept, but they should not dominate the final verdict or the result page.

Third, article preprocessing needs to be stronger. In the ISOT-style example, fragments such as `do and he couldn t do it. As` and joined text such as `2017Trump s tweet` indicate that the input may contain copied social-media text, missing apostrophes, missing spaces, URLs, or quote boundaries. The current sentence splitter is intentionally simple, but long article input may need a cleaning step before atomization so that sentence boundaries and speaker/context cues are not lost.

Fourth, duplicate or near-duplicate claims can appear after atomization. This makes the result page longer and can over-weight the same factual point in the final aggregate. A future atomizer cleanup should merge obvious duplicate claims or keep only one representative claim before fact checking.

Finally, the frontend should not expose raw missing verdict values such as `Verdict: N/A` or `Truth Score: N/A` as if they were model outputs. These usually mean the backend returned `degraded`, `no_evidence`, or `insufficient_evidence` without a final score. For users, this should be displayed in product language such as `No final verdict` or `Insufficient evidence`, while still keeping evidence sufficiency visible.

### Atomizer and Query Rewrite Boundary

A later pipeline review clarified that the system should separate two tasks that were previously both described as "rewrite".

The first task is claim preparation. This belongs to the atomizer. The atomizer receives the user text and produces standalone factual claims that can be searched and checked. This includes preserving attribution or belief framing, splitting multi-fact sentences, and resolving clear local references such as `he`, `it`, `this`, or `that` when the nearby context identifies the subject or object. For example:

| Input context | Atomized factual claim |
|---|---|
| `Donald Trump posted a message on Twitter. He said the stock market had its best year.` | `Donald Trump said the stock market had its best year.` |
| `Donald Trump praised the tax bill. He said it would help workers.` | `Donald Trump said the tax bill would help workers.` |
| `People once thought the Sun orbits around Earth.` | `People once thought the Sun orbits around Earth.` |

The second task is retrieval query rewriting. This belongs to the fact-checking recovery layer, not to the atomizer. Once the atomizer has produced a standalone claim, fact-checking should first search using that claim. If the first search returns no raw evidence, recovery may generate an alternative search query when the user enables rewrite fallback. That fallback query is a search aid, not a new factual claim.

This gives a cleaner pipeline boundary:

1. Atomizer: user text -> standalone factual claims.
2. Fact-checking: standalone factual claim -> retrieve evidence -> filter evidence -> judge evidence.
3. Recovery: empty search or unstable judgment -> narrow fallback/rerun.

This also affects frontend wording. The user-facing result should show the factual `Claim`, while retrieval metadata should be labelled as `Search Query` or kept as technical detail. A search query should not be presented as if it were the factual claim itself.

The code was then cleaned to match this boundary. `entities`, `relation`, and `constraints` are now top-level fields on `EachFactualClaim`, because they describe the checkable claim itself. Retrieval metadata is kept only for runtime details such as the query used, fallback status, evidence counts, and Gemini calibration score.

The main fact-checking path no longer calls Gemini to prepare or rewrite the claim. It only rejects empty or extremely short input, then searches the atomizer's checkable claim directly. Gemini rewrite remains only as optional recovery for empty search. This keeps the normal path shorter and makes the pipeline easier to explain:

```text
atomizer checkable claim -> Tavily/NLI retrieval -> Gemini evidence judgment -> backend aggregation
```

Recovery and stabilization were also narrowed so they do not look like a second hidden fact-checking pipeline:

- rewrite fallback is only for empty search;
- post-judgment recovery only tries a remaining query when selected evidence was judged as background-only;
- stabilization only reruns borderline or low-confidence Gemini judgment and then pulls unstable scores toward neutral.

This is intentionally conservative. It avoids repeatedly rewriting or searching a claim just because the final verdict is not ideal, while still giving the system one simple recovery path for the cases that are most clearly retrieval-related.

### Atomizer Local Test Notes

A local atomizer-only test was added to inspect Gemini atomization without running Tavily, NLI, text-pattern scoring, or the full backend pipeline. The test confirmed several improvements after the atomizer prompt and validation cleanup:

- multi-fact sentences can be split into separate factual claims;
- subjective/non-factual sentences can be ignored while nearby factual sentences remain;
- clear local pronouns can be resolved into standalone claims;
- belief/attribution framing is preserved instead of being reduced to the embedded proposition;
- exact duplicate claims can be collapsed so they are not checked twice.

The same test also showed a reasonable remaining limitation. Messy copied article text such as social media handles, missing spaces, and compacted dates can still produce awkward sentence boundaries. The current safer behavior is to ignore unclear fragments rather than send incomplete claims like `He couldn't do it` into fact-checking. A later article-level preprocessing step can improve this, but the current atomizer is moving in the right direction for claim-level reliability.
