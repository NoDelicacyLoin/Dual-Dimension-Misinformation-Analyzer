...
def analyze_page_shell_features(search_query: str, evidence_url: str, evidence_text: str) -> bool:
    ...
    looks_like_listing_page = (
        ...
        or (title_heading_count >= 3 and (strong_listing_marker_count >= 1 or weak_listing_marker_count >= 1))
    ...
    return word_count < 10 and (navigation_marker_count >= 1 or link_count >= 3)
...
def fetch_oversampled_evidence(search_query: str, max_results: int = 8) -> SearchResult:
    ...
    # Block domains that are not suitable as product-facing evidence sources.
    unreliable_domains = [
