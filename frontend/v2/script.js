...
function makeClaimBlock(claimGroup, index) {
    ...
    return (
        ...
        "<span>Overall Verdict: " + escapeHtml(factVerdict || "N/A") + "</span>" +
        "<span>Text-Pattern Risk: " + formatScoreLabel(textRiskScore) + "</span>" +
        "<span>Fact-Checking Support: " + formatScoreLabel(factTruthScore) + "</span>" +
        "</div>" +
        "</header>" +
...
function makeFactualClaimBlock(factualClaim, index) {
    ...
    return (
        ...
        "<span>Verdict: " + escapeHtml(verdictLabel) + "</span>" +
        "<span>Truth Score: " + escapeHtml(truthScoreLabel) + "</span>" +
        "<span>Confidence: " + escapeHtml(decisionConfidenceLabel) + "</span>" +
        "<span>Evidence Sufficiency: " + escapeHtml(evidenceSufficiencyLabel) + "</span>" +
        "</div>" +
        "</section>" +
...
function makeEvidenceBlocks(evidenceItems) {
    ...
    return evidenceItems.map(function (evidenceItem) {
        ...
        return (
            ...
            '<span class="evidence_tag evidence_stance ' + stanceClassName + '">Stance: ' + escapeHtml(getEvidenceStanceLabel(stance)) + "</span>" +
            '<span class="evidence_tag evidence_quality">Evidence Quality: ' + escapeHtml(getStatusLabel(evidenceItem.evidence_quality || "unknown")) + "</span>" +
            "</div>" +
...
function makeScoreRing(title, score, label) {
    ...
    return (
        ...
        "<span>" + escapeHtml(label || "N/A") + "</span>" +
        "</div>" +
        "</div>" +
...
function getFactualClaimVerdictLabel(factualClaim) {
    ...
    if (factualClaim.status === "no_evidence") {
        return "No Evidence Found";
    }

    if (factualClaim.status === "invalid_request") {
        return "Not Checkable";
    }

    if (factualClaim.status === "system_error") {
        return "System Error";
    }

    if (factualClaim.status === "degraded") {
        return "Degraded";
    }
...
function getRiskLevelLabel(riskLevel) {
    ...
    return getStatusLabel(riskLevel);
}
...
function getConfidenceDisplayLabel(confidenceLevel) {
    ...
    return getStatusLabel(confidenceLevel);
}
